...
namespace UnityStandardAssets.CrossPlatformInput
{
    ...
    public class TiltInput : MonoBehaviour
    {
		...
		[Range(1f, 100f)] public float sensitivity = 10f;
        ...
        private void OnEnable()
        {
			if (visualSlider != null){
				visualSlider.minValue = -1f;
				visualSlider.maxValue = 1f;
				visualSlider.value = 0f;
			}
        ...
        private void Update()
        {
			...
			float axisValue = Mathf.Round((Mathf.InverseLerp(-fullTiltAngle, fullTiltAngle, angle) * 2 - 1) * sensitivity) / sensitivity;
			visualSlider.value = Mathf.Lerp(visualSlider.value, axisValue, 0.1f);
            switch (mapping.type)
            {
